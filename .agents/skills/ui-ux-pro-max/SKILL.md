...
- `container-width` - Consistent max-width on desktop (max-width / max-w-7xl)
...
- `number-tabular` - Use tabular figures for data
...
- `autofill-support` - Use autocomplete
- `undo-support` - Allow undo
- `success-feedback` - Confirm completed actions
- `error-recovery` - Explain how to fix
- `multi-step-progress` - Show step indicator
- `form-autosave` - Auto-save drafts
- `destructive-emphasis` - Destructive actions use semantic red color
- `toast-accessibility` - Use aria-live

### 9. Navigation Patterns (HIGH)

- `bottom-nav-limit` - Bottom navigation max 5 items
- `drawer-usage` - Use drawer for secondary navigation
- `back-behavior` - Make back navigation predictable
- `deep-linking` - Keep screens reachable via URL
- `nav-label-icon` - Nav items must have icon and label
- `nav-state-active` - Highlight current location
- `modal-escape` - Modals offer a close affordance
- `breadcrumb-web` - Web: use breadcrumbs
- `state-preservation` - Navigating back restores scroll position

### 10. Charts & Data (LOW)

- `chart-type` - Match chart type to data type
- `color-guidance` - Accessible color palettes
- `data-table` - Provide table alternative
- `legend-visible` - Always show legend
- `tooltip-on-interact` - Provide tooltips on hover
- `empty-data-state` - Show meaningful empty state

## How to Use

*(CLI tools referenced here are abstract. Incorporate these guidelines manually in agent thoughts/responses).* Ensure rules related to spacing, icons, interaction states, accessibility, text sizes, layout and animations are strictly adhered to.
