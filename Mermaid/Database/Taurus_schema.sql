...
-- Version: 3.0 - Enhanced with Full Product Catalog
--           from PARTS-PRICES.xlsx
-- Changes from v2.0:
--   + Added Brand table (new)
--   + Added CategoryCode to Category table
--   + Added 12 bicycle-specific columns to Product table
--     (SKU, BrandId, Material, Color, WheelSize,
--      SpeedCompatibility, BoostCompatible, TubelessReady,
--      AxleStandard, SuspensionTravel, BrakeType,
--      AdditionalSpecs)
--   + Added PriceHistory table (new)
--   + Seed data: 13 Categories, 43 Brands, 148 Products
--   + All v2.0 tables, views, indexes, and roles preserved
-- =============================================
...
IF EXISTS (SELECT name FROM sys.databases WHERE name = N'TaurusBikeShop')
BEGIN
    ALTER DATABASE TaurusBikeShop SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
    DROP DATABASE TaurusBikeShop;
END
GO

CREATE DATABASE TaurusBikeShop;
GO
...
CREATE OR ALTER TRIGGER TR_Product_PriceAudit
ON Product
AFTER UPDATE
AS
BEGIN
...
CREATE TABLE Payment (
    ...
    CONSTRAINT CK_Payment_Method        CHECK (PaymentMethod IN ('GCash','Cash','Card')),
...
CREATE TABLE Delivery (
    ...
    OrderId                 INT             NOT NULL,
    LalamoveReference       NVARCHAR(255)   NULL,
...
CREATE INDEX IX_Delivery_LalamoveReference  ON Delivery(LalamoveReference);
GO
...
-- =============================================
-- Categories  (from Excel column headers)
-- =============================================
...
-- =============================================
-- Brands  (derived from product names in Excel
--          and enriched via web research)
-- =============================================
...
-- =============================================
-- Products  (148 rows from PARTS-PRICES.xlsx,
...
PRINT '==============================================';
PRINT 'TaurusBikeShop v3.0 — Database Ready!';
PRINT '==============================================';
PRINT 'Tables Created  : 26 (24 original + Brand + PriceHistory)';
PRINT 'Views Created   : 6 (all updated to include Brand columns)';
PRINT 'Triggers Created: 1 (TR_Product_PriceAudit)';
PRINT 'Indexes Created : 95+';
PRINT '----------------------------------------------';
PRINT 'Seed Data Inserted:';
PRINT '  Roles      :  5';
PRINT '  Categories : 13  (from Excel column headers)';
PRINT '  Brands     : 43  (derived + web-researched)';
PRINT '  Products   : 148 (enriched from PARTS-PRICES.xlsx)';
PRINT '----------------------------------------------';
PRINT 'New in v3.0 vs v2.0:';
PRINT '  + Brand table with 43 records';
PRINT '  + PriceHistory table + auto-audit trigger';
PRINT '  + CategoryCode column on Category';
PRINT '  + Product.BrandId  FK -> Brand';
PRINT '  + Product.SKU      (unique, e.g. UNIT-PIN-001)';
PRINT '  + Product.ShortDescription';
PRINT '  + Product.Currency (default PHP)';
PRINT '  + Product.Material';
PRINT '  + Product.Color';
PRINT '  + Product.WheelSize';
PRINT '  + Product.SpeedCompatibility';
PRINT '  + Product.BoostCompatible';
PRINT '  + Product.TubelessReady';
PRINT '  + Product.AxleStandard';
PRINT '  + Product.SuspensionTravel';
PRINT '  + Product.BrakeType';
PRINT '  + Product.AdditionalSpecs';
PRINT '  + vw_ActiveProducts updated (Brand, CategoryCode)';
PRINT '  + vw_InventoryStatus updated (BrandName)';
PRINT '==============================================';
GO
