...
public sealed class SupportCreateViewModel
{
    ...
    /// <summary>
    /// Ticket category — must match a <c>TicketCategories</c> constant.
    /// Required on POST.
    /// </summary>
    ...
    /// <summary>
    /// Brief subject line (max 200 chars). Required on POST.
    /// </summary>
    [Required(ErrorMessage = "Please provide a subject.")]
    [MaxLength(200, ErrorMessage = "Subject must be 200 characters or fewer.")]
    public string? Subject { get; set; }

    /// <summary>Full description of the issue. Optional.</summary>
    [MaxLength(5000, ErrorMessage = "Description must be 5 000 characters or fewer.")]
    public string? Description { get; set; }

    /// <summary>
    /// Optional FK to a related order.
    /// Submitted as a hidden field when navigating from an order detail page.
    /// </summary>
    public int? OrderId { get; set; }

    // ── Reference data (GET only) ─────────────────────────────────────────────

    /// <summary>
    /// All valid ticket categories for the category drop-down.
    /// Populated from <c>TicketCategories</c> constants.
    /// </summary>
    ...
    /// <summary>
    /// Pre-selected order number shown in the form when coming from an order page.
    /// Display only — the FK is carried by <see cref="OrderId"/>.
    /// </summary>
    public string? OrderNumber { get; set; }
}
